(function () {
  ...
  var createServer = require('../main/createServer');

  describe('createServer', function () {
    var server, logger, http, httpServer = null;
    ...
    describe('createServer()', function () {
      ...
      describe('passing a logger', function () {
        it('returns a initialized Server instance', function (done) {
          ...
          expect(server._appInfo).to.be.null;
          expect(server._config).to.be.empty;
          expect(server._logger.logLevel.called).to.be.false;
          done();
        });
      });
      ...
      describe('with config file', function (done) {
        it('loads the config and sets the logLevel', function (done) {
          var fakeData = '{ "development": { "logLevel": "debug"} }';
          var fs = require('fs');
          ...
          expect(server._logger.logLevel.called).to.be.true;
          expect(server._logger.logLevel.getCall(0).args[0]).to.be.equal('debug');
          done();
        });
      });
    ...
    describe('server.startup()', function () {
      describe('startup()', function (done) {
        it('starts server', function (done) {
          ...
          expect(server._appInfo.name).to.be.equal('i8');
          expect(server._appInfo.version).to.be.defined;
          done();
        });
      });
